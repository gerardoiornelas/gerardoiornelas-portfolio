...
Usage tracking surfaces: the automatic usage ledger lives in `scripts/uig-usage.js` (reader) and `scripts/uig-stop-hook.js` (Claude Code SessionEnd hook parsing the transcript). The skill writes invocation/completion events to the global `~/.uig/tracking.jsonl`; the hook appends per-session token lines and is registered in `~/.claude/settings.json`. Keep the event schema in `scripts/uig-usage.js` aligned with the skill's Automatic tracking section.
