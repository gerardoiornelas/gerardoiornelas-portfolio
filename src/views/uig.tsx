...
6. Execute authorized work within scope. Gated actions require explicit principal approval before execution. Prohibited actions stop; propose a permissible alternative.
...
Reuse valid authorization within its scope; recheck on changed scope, actor, conditions, expiry, or revocation. Acceptance records evidence after execution and never retroactively authorizes it. Changes to governing rules, permissions, verification requirements, or completion criteria require explicit approval.
Read required context, then a compact lesson index when available; load only applicable lessons and affected source. Expand retrieval when evidence is missing. Preserve required graph refreshes after material changes. Deduplicate evidence-backed lessons with applicability and limits. No new lesson and cause unknown are valid outcomes. Measure total tokens per accepted task, including retries and learning overhead; preserve required checks.
