...
describe("VerificationDemo", () => {
  ...
  it("switches scenarios and renders the expired verification state", () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: /scoped payment settlement/i })
    )
    expect(
      screen.getByText("Settle a contractor milestone")
    ).toBeInTheDocument()

    fireEvent.click(screen.getByRole("button", { name: /verify receipt/i }))

    expect(
      screen.getByText("Expired before safe settlement")
    ).toBeInTheDocument()
  ...
  it("renders revoked state distinctly from confirmed state", () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: /sensitive workflow access/i })
    )
    fireEvent.click(screen.getByRole("button", { name: /verify receipt/i }))
