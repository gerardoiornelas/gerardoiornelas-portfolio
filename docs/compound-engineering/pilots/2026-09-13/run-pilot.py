...
if sys.argv[1:] != ['--approved-payload', expected]:
 raise SystemExit('Explicit approval of the reviewed payload is required before running.')
...
'docs': 'Align Gatsby build troubleshooting instructions in AGENTS.md, knowledge/context.md, and knowledge/README.md. State the complete command GATSBY_TELEMETRY_DISABLED=1 GATSBY_FEEDBACK_DISABLED=1 npm run build, explain telemetry and feedback are separate, and make no broader permission or validation changes. Acceptance: all three files contain the complete command and explanation; npm run okf:validate passes; no application code changed.'}
common='''This is one authorized local experimental ticket in a disposable copy. Read WORKFLOW.md as the selected workflow, then required repository instructions and task-relevant source. The selected WORKFLOW.md controls retrieval for this trial; other copies of the skill are source artifacts, not a second workflow to load. Do not consult sibling runs, the source repository, or other agents. Use only local shell/file tools. Do not install packages, use network/browser/connectors, spawn agents, change tool config, commit, push, or deploy. node_modules is shared read-only; do not modify it.
...
Write one receipt from knowledge/templates/template-receipt.md and only warranted learning; do not fabricate token counts. End with a concise final containing changes, check results, limitations and pending human acceptance if any. Do not edit WORKFLOW.md or this experimental protocol.
Task: '''
runs=[]
for rep in [1,2]:
 for task in tasks:
  for variant in (['baseline','compact'] if rep==1 else ['compact','baseline']):
   ...
   # All source code stays equal. Baseline receives the prior workflow and no learned shortcuts.
   (work/'WORKFLOW.md').write_text(old_skill if variant=='baseline' else new_skill)
   if variant=='baseline':
    context=(work/'knowledge/context.md').read_text().split('## Compact task retrieval')[0]
    (work/'knowledge/context.md').write_text(context)
    shutil.rmtree(work/'knowledge/lessons')
    (work/'knowledge/lessons').mkdir();(work/'knowledge/lessons/index.md').write_text('# Lesson index\nNo pilot lessons available in this baseline.\n')
