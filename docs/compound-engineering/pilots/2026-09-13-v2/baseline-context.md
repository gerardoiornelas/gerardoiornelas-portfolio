...
Run `npm run build` for meaningful site changes. The Gatsby build may need telemetry disabled in restricted environments: `GATSBY_TELEMETRY_DISABLED=1 GATSBY_FEEDBACK_DISABLED=1 npm run build`.
...
After required instructions, read `knowledge/lessons/index.md` and only matching lessons, then affected source and dependencies. Expand when evidence is missing. Full receipt history is not startup context. Preserve required graph refreshes after material changes. Use `knowledge/templates/template-receipt.md` for new/modified committed receipts; unchanged historical receipts are grandfathered. Structural validation cannot prove authorization or evidence truth.
