...
report={'rows':rows,'pairs':pairs,'limitations':['UI human acceptance is separate and may remain pending.','Graph semantic refresh failed on the installed backend; full operational acceptance is not established.','Current implementation code and acceptance tests were held equal; startup routing and receipt-authoring method differed; lessons and receipt schema were held equal.','Two repetitions are exploratory, not statistical proof.','Parent orchestration, initial development, and reviewer token use are not included in worker totals; this is not end-to-end organizational token accounting.','Failed command counts include intentional red tests.','Cached and reasoning tokens are subsets, not added twice.']}
