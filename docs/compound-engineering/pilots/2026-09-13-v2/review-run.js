...
const ROOT='/tmp/uig-token-comparison-v2';
...
for(const run of runs){
 ...
 if(run.task==='docs'){
  const sentence='Use a temporary XDG_CONFIG_HOME only after a global configuration write failure; preserve the sandbox and required checks.';
  result.checks.exact_guidance=allowed.every(f=>fs.readFileSync(cwd+'/'+f,'utf8').split(sentence).length===2);
  result.checks.existing_docs_preserved=allowed.every(f=>{
   const before=cp.execFileSync('git',['show','HEAD:'+f],{cwd,encoding:'utf8'});
   let after=fs.readFileSync(cwd+'/'+f,'utf8').replace(sentence,'');
   if (!before.includes('## Build troubleshooting')) after=after.replace(/^## Build troubleshooting\s*$/m,'');
   const original=before.replace(/\s+/g,''); let cursor=0;
   for (const char of after.replace(/\s+/g,'')) if(char===original[cursor]) cursor++;
 ...
 result.checks.receipt_method=run.variant==='compact' ? commands.some(i=>/okf:receipt/.test(i.shell||'')&&i.exit_code===0&&/Created knowledge\/receipts\//.test(i.aggregated_output||'')) : !commands.some(i=>/okf:receipt|node[^\n]*create-okf-receipt\.js/.test(i.shell||''));
