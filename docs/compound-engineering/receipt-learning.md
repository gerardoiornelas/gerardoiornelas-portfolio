...
A lesson's state is derived, never read from the last history file. History is plain files, so `state`, `retrieve`, `plan`, and every receipt attempt replay a lesson's history and accept each step only when the artifact the real transition leaves behind supports it. `evaluated` needs a stored evaluation report for the stored plan (its hash must match) with `transfer_supported` and at least two `lesson_transfer_pairs` for that lesson; a plan that merely lists a co-selected lesson does not vouch for it. `approved` needs a prior `evaluated` state and an approval bound to the lesson's content hash with principal, source, scope, and an expiry. `retired` needs a reason and is terminal. Anything else makes the lesson `invalid-history`: it is never retrieved, planned, supplied, or approved, and `stats` counts it. This raises the bar from writing one file to forging a mutually consistent set of files; it does not authenticate the principal, so approvals remain local attestations that reviewers must inspect.
